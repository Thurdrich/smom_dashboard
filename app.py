...
def _clean_columns(data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Interpolate only values for which interpolation is meaningful. Do not
    # manufacture categorical passenger details; those remain explicitly Unknown.
    ...
    if "STATUS" in data:
        data["STATUS"] = data["STATUS"].replace({"": "UNKNOWN", "NAN": "UNKNOWN", "NONE": "UNKNOWN"}).str.upper()
    return data
...
def text_col(data, col):
    ...
    return data[col].fillna("Unknown").astype(str).str.upper()
...
def apply_filters(data):
    with st.sidebar:
        ...
        sex = choose("Sex", "SEX")
        st.divider()
        st.caption("All passengers in the attached Epic Fury Data file are included by default.")
...
def answer_question(question, data):
    ...
    if not q:
        return "Ask me about passenger counts, statuses, locations, vessels, or dates."
    ...
    return "I can answer questions using the currently filtered Epic Fury passenger data."


df = load_data()
if df is None:
    st.error(f"The sole dashboard source, `{SOURCE_FILE}`, is missing or empty. Add the attached passenger data to that file; no other repository dataset is used.")
    st.stop()
...
with tab_roster:
    ...
    preferred = ["STATUS", "EXT_STATUS", "PERSON", "SEX", "RATING", "VESSEL", "LOCATION", "START", "IN_OUT", "DATE_1", "DATE_2", "DOA", "DUE OFF DT", "COMMENTS"]
    columns = [c for c in preferred if c in filtered] or list(filtered.columns)
...
with tab_chat:
    ...
    st.caption("Answers are generated only from the attached Epic Fury Data file.")
