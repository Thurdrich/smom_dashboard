...
def demo_data():
    ...
    staffing = (workload * rng.normal(0.91, 0.12, 180)).clip(5, 150).round(1)
    return pd.DataFrame(
        {
            "Date": dates,
            ...
            "Priority": rng.choice(["Routine", "Elevated", "Critical"], 180, p=[0.55, 0.3, 0.15]),
        }
    )
...
def parse_dates(series):
    ...
    if parsed.notna().mean() < 0.7 and pd.api.types.is_numeric_dtype(clean):
...
def detect_sensitive(data):
    ...
    for column in data.columns:
        ...
        if len(sample):
            if sample.str.contains(r"(?:\+?\d[\d .()\-]{7,}\d)", regex=True).mean() >= 0.15:
                reason.append("phone-like values")
            if sample.str.contains(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", regex=True).mean() >= 0.15:
                reason.append("email-like values")
            if sample.str.contains(r"^(?:[A-Z]{0,4}[- ]?\d{5,}|\d{6,})$", regex=True).mean() >= 0.15:
...
def profile(data):
    ...
    for column in data.columns:
        ...
        label = str(column).lower()
        date_likely = any(word in label for word in DATE_WORDS) and date_candidate.notna().mean() >= 0.35
        if date_likely or (date_candidate.notna().mean() >= 0.85 and numeric_candidate.notna().mean() < 0.85 and non_null.nunique() > 1):
            dates.append(column)
        elif pd.api.types.is_numeric_dtype(series) or numeric_candidate.notna().mean() >= 0.9:
            numeric.append(column)
        elif non_null.nunique() <= min(30, max(10, len(data) * 0.2)):
            categorical.append(column)
        else:
            text.append(column)
